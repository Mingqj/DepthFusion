...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='/opt/data/private/codeN1/mmdetection3d/data/nuscenes/',
        help='Path to nuScenes dataset')
    parser.add_argument(
        '--save_path',
        type=str,
        # default='./vis',
        default="/opt/data/private/codeN1/BEVDet/Save/Bbox2",
...
# color_map = {0: (255, 255, 0), 1: (0, 255, 255)} # yellow green
# color_map = {0: (153, 51, 0), 1: (255, 20, 147)} # yellow green
color_map = {0: (0, 255, 0), 1: (255, 100, 0)} # yellow green
...
def main():
    ...
    info_path = \
        args.root_path + '/full-bevdetv3-nuscenes_infos_%s.pkl' % args.version
    ...
    for cnt, infos in enumerate(
            ...
            for aid in range(valid.shape[0]):
                ...
                for index in draw_boxes_indexes_img_view:
                    if (valid[aid, index[0]] and valid[aid, index[1]]):
                        # print(corners_img[aid, index[0]], corners_img[aid, index[1]])
                        cv2.line(
                            img,
        ...
        for rid in sort_ids:
            score = scores[rid]
            if score < args.vis_thred and pred_flag[rid]: # for example: 0.82, True; 0.0, Flase
                continue
