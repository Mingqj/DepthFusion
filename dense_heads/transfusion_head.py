...
iteration = 0
def draw_bev(x, iteration, type=None):     # x:tensor(C,H,W)
    import matplotlib.pyplot as plt
    feat_map = x[0].max(0)[0].cpu().detach().numpy()
    fig = plt.figure(figsize=(20, 20))
    ax = fig.add_subplot(111)
    im = ax.imshow(feat_map, cmap=plt.cm.jet)
    plt.axis('off')
    plt.savefig("/opt/data/private/codeN1/BEVDet/Save/BEV_feature/" + type + str(iteration) + ".jpg")
    plt.show()
...
class TransFusionHead(nn.Module):
    def __init__(
        ...
        bias="auto",
        # loss
        loss_cls0=dict(type="GaussianFocalLoss", reduction="mean"),
        loss_cls=dict(type="GaussianFocalLoss", reduction="mean"),
        loss_iou=dict(
            type="VarifocalLoss", use_sigmoid=True, iou_weighted=True, reduction="mean"
        ),
        loss_bbox0=dict(type="L1Loss", reduction="mean"),
        loss_bbox=dict(type="L1Loss", reduction="mean"),
        loss_heatmap=dict(type="GaussianFocalLoss", reduction="mean"),
        loss_heatmap_attention=dict(type="GaussianFocalLoss", reduction="mean"),
        loss_heatmap_sparse=dict(type="GaussianFocalLoss", reduction="mean"),
        # others
        train_cfg=None,
        test_cfg=None,
        bbox_coder=None,
        ...
        self.loss_cls0 = build_loss(loss_cls0)
        self.loss_bbox0 = build_loss(loss_bbox0)
        self.loss_iou = build_loss(loss_iou)
        self.loss_heatmap = build_loss(loss_heatmap)
        self.loss_heatmap_attention = build_loss(loss_heatmap_attention)
        self.loss_heatmap_sparse = build_loss(loss_heatmap_sparse)
        ...
        self.prediction_heads0 = nn.ModuleList()
        for i in range(self.num_decoder_layers):
            heads = copy.deepcopy(common_heads0)
            heads.update(dict(heatmap0=(self.num_classes, num_heatmap_convs)))
            self.prediction_heads0.append(
                FFN(
                    hidden_channel,
                    heads,
                    conv_cfg=conv_cfg,
    ...
    def get_targets0(self, gt_bboxes_3d, gt_labels_3d, preds_dict):
    ...
    def get_targets_single0(self, gt_bboxes_3d, gt_labels_3d, preds_dict, batch_idx):
        ...
        """
        num_proposals = preds_dict["center0"].shape[-1]

        # get pred boxes, carefully ! donot change the network outputs
        score = copy.deepcopy(preds_dict["heatmap0"].detach())
        center = copy.deepcopy(preds_dict["center0"].detach())
        height = copy.deepcopy(preds_dict["height0"].detach())
        dim = copy.deepcopy(preds_dict["dim0"].detach())
        rot = copy.deepcopy(preds_dict["rot0"].detach())
        if "vel0" in preds_dict.keys():
            vel = copy.deepcopy(preds_dict["vel0"].detach())
        else:
            vel = None
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        (
            ...
            matched_ious,
            heatmap,
        ) = self.get_targets0(gt_bboxes_3d, gt_labels_3d, preds_dicts[0])
        ...
        # # compute heatmap loss in global fusion
        # loss_heatmap_attention = self.loss_heatmap_attention(
        #     clip_sigmoid(preds_dict["dense_heatmap_attention"]),
        #     heatmap,
        #     avg_factor=max(heatmap.eq(1).float().sum().item(), 1),
        # )
        # loss_dict["loss_heatmap_attention"] = loss_heatmap_attention

        # compute heatmap loss in sparse fusion
        loss_heatmap_sparse = self.loss_heatmap_sparse(
            clip_sigmoid(preds_dict["dense_heatmap_sparse"]),
            heatmap,
            avg_factor=max(heatmap.eq(1).float().sum().item(), 1),
        )
        loss_dict["loss_heatmap_sparse"] = loss_heatmap_sparse
        ...
        for idx_layer in range(self.num_decoder_layers if self.auxiliary else 1):
            ...
            layer_labels0 = labels[
                ...,
                idx_layer * self.num_proposals : (idx_layer + 1) * self.num_proposals,
            ].reshape(-1)
            layer_label_weights0 = label_weights[
                ...,
                idx_layer * self.num_proposals : (idx_layer + 1) * self.num_proposals,
            ].reshape(-1)
            layer_score0 = preds_dict["heatmap0"][
                ...,
                idx_layer * self.num_proposals : (idx_layer + 1) * self.num_proposals,
            ]
            layer_cls_score0 = layer_score0.permute(0, 2, 1).reshape(-1, self.num_classes)
            layer_loss_cls0 = self.loss_cls0(
                layer_cls_score0,
                layer_labels0,
                layer_label_weights0,
                avg_factor=max(num_pos, 1),
            )

            layer_center0 = preds_dict["center0"][
                ...,
                idx_layer * self.num_proposals : (idx_layer + 1) * self.num_proposals,
            ]
            layer_height0 = preds_dict["height0"][
                ...,
                idx_layer * self.num_proposals : (idx_layer + 1) * self.num_proposals,
            ]
            layer_rot0 = preds_dict["rot0"][
                ...,
                idx_layer * self.num_proposals : (idx_layer + 1) * self.num_proposals,
            ]
            layer_dim0 = preds_dict["dim0"][
                ...,
                idx_layer * self.num_proposals : (idx_layer + 1) * self.num_proposals,
            ]
            preds0 = torch.cat(
                [layer_center0, layer_height0, layer_dim0, layer_rot0], dim=1
            ).permute(
                0, 2, 1
            )  # [BS, num_proposals, code_size]
            if "vel0" in preds_dict.keys():
                layer_vel0 = preds_dict["vel0"][
                    ...,
                    idx_layer
                ...
                preds0 = torch.cat(
                    [layer_center0, layer_height0, layer_dim0, layer_rot0, layer_vel0], dim=1
                ).permute(
                    0, 2, 1
                )  # [BS, num_proposals, code_size]
            code_weights0 = self.train_cfg.get("code_weights", None)
            layer_bbox_weights0 = bbox_weights[
                :,
                idx_layer * self.num_proposals : (idx_layer + 1) * self.num_proposals,
                :,
            ]
            layer_reg_weights0 = layer_bbox_weights0 * layer_bbox_weights0.new_tensor(
                code_weights0
            )
            layer_bbox_targets0 = bbox_targets[
                :,
                idx_layer * self.num_proposals : (idx_layer + 1) * self.num_proposals,
                :,
            ]
            layer_loss_bbox0 = self.loss_bbox0(
                preds0, layer_bbox_targets0, layer_reg_weights0, avg_factor=max(num_pos, 1)
            )
            ...
            loss_dict[f"{prefix}_loss_cls"] = layer_loss_cls0
            loss_dict[f"{prefix}_loss_bbox"] = layer_loss_bbox0
            # loss_dict[f'{prefix}_loss_iou'] = layer_loss_iou
        loss_dict[f"matched_ious0"] = layer_loss_cls0.new_tensor(matched_ious)

        ############################################################
        # global iteration
        # draw_bev(heatmap, iteration, type='gt')
        # iteration += 1
