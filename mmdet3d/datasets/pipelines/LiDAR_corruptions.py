...
def rain_sim_lidar(pointcloud,severity):
    from mmdet3d.datasets.pipelines.utils import lisa
...
Snow
'''
# def snow_sim_lidar(pointcloud,severity):
#     from mmdet3d.datasets.pipelines.utils import lisa
#     from mmdet3d.datasets.pipelines.utils.wet_ground.augmentation import ground_water_augmentation
...
def snow_sim_lidar(pointcloud,severity):
    ...
    '''
    import sys
    sys.path.append('./LiDAR_snow_lidar/')
    from mmdet3d.datasets.pipelines.utils.snowfall.simulation import augment
    from mmdet3d.datasets.pipelines.utils.wet_ground.augmentation import ground_water_augmentation
    # from mmdet3d.datasets.pipelines.utils.snowfall.sampling import snowfall_rate_to_rainfall_rate, compute_occupancy
...
Fog
'''
def fog_sim_lidar(pointcloud,severity):
    from mmdet3d.datasets.pipelines.utils.fog_sim import simulate_fog
    from mmdet3d.datasets.pipelines.utils.fog_sim import ParameterSet
...
def moving_noise_bbox(pointcloud,severity,bbox):
    cor = 'move_bbox'
    from mmdet3d.datasets.pipelines.utils import bbox_util
...
def fulltrajectory_noise(pointcloud, pc_pose, severity):
    from mmdet3d.datasets.pipelines.utils.lidar_split import lidar_split, reconstruct_pc_cpu
    ...
    f_pc = reconstruct_pc_cpu(new_lidar_list, new_pose_list)
    return f_pc
