...
import torch
import cv2
from .LiDAR_corruptions import gaussian_noise,lidar_crosstalk_noise,density_dec_global,density_dec_local,cutout_local,uniform_noise,impulse_noise,scene_glare_noise,cutout_bbox,density_dec_bbox,gaussian_noise_bbox,scale_bbox,shear_bbox,moving_noise_bbox,uniform_noise_bbox,impulse_noise_bbox,rotation_bbox,spatial_alignment_noise,fulltrajectory_noise, temporal_alignment_noise, fov_filter, rain_sim_lidar, fog_sim_lidar, snow_sim_lidar

# background_noise, upsampling, layer_del, FFD_bbox, upsampling_bbox, filter_point_by_angle
...
class CorruptionMethods(object):
    ...
    def __init__(self,
        ...
        if 'snow_sim_image' in self.corruption_severity_dict:
            # 注意这个只是加图像噪声，没有点云干扰
            severity = self.corruption_severity_dict['snow_sim_image']
            self.snow_sim = ImageAddSnow(severity, seed=2022)

        if 'fog_sim_image' in self.corruption_severity_dict:
            # 注意这个只是加图像噪声，没有点云干扰
            severity = self.corruption_severity_dict['fog_sim_image']
            self.fog_sim = ImageAddFog(severity, seed=2022)
            
        if 'rain_sim_image' in self.corruption_severity_dict:
            # 注意这个只是加图像噪声，没有点云干扰
            severity = self.corruption_severity_dict['rain_sim_image']
    ...
    def __call__(self, results):
        ...
        if 'sun_sim' in self.corruption_severity_dict:
            ...
            for cam_name in cam_names:
                ...
                # filename = '/opt/data/common/nuScenes_org/data_v1.0-trainval/' +  filename[53:]
                img = Image.open(filename)
                post_rot = torch.eye(2)
                post_tran = torch.zeros(2)
                img_augs = sample_augmentation(
                    H=img.height, W=img.width, flip=None, scale=None)
                resize, resize_dims, crop, flip, rotate = img_augs
                img, post_rot2, post_tran2 = img_transform(img, post_rot,
                                                    post_tran,
                                                    resize=resize,
                                                    ...
                                                    flip=flip,
                                                    rotate=rotate)
                # cv2.imwrite("/gongshenjian/jimingqian/BEVDet/Save/3.jpg", img)
                # if cam_name == 'CAM_FRONT_LEFT':
                if cam_name == 'CAM_BACK_RIGHT':
                    new_np = img[:,:,[2,1,0]].astype(np.uint8)
                    # lidar2img_0 = lidar2img[0]
                    image_aug_rgb_0 = self.sun_sim(
                        image=new_np,
                    ...
                    image_aug_bgr_0 = image_aug_rgb_0[:, :, [2, 1, 0]].astype(np.float32)
                    img = image_aug_bgr_0
                    # cv2.imwrite("/gongshenjian/jimingqian/BEVDet/Save/4.jpg", image_aug_bgr_0)
                    imgs.append(mmlabNormalize(image_aug_bgr_0))
                else:
                    imgs.append(mmlabNormalize(img))

                filename1 = results['curr']['cams']['CAM_FRONT']['data_path']
                cv2.imwrite("/opt/data/private/codeN1/BEVDet1/Save/curruptions/sun_image/" + filename1[filename1.rfind("/") + 1:-4] + cam_name + ".jpg", img)

            imgs = torch.stack(imgs)
            results['img_inputs'] = tuple([imgs, results['img_inputs'][1], results['img_inputs'][2], results['img_inputs'][3], results['img_inputs'][4], results['img_inputs'][5]])
            # results['points'].tensor = points_aug
        ...
        #     img_bgr_255_np_uint8 = results['img_inputs']
        #     img_rgb_255_np_uint8 = img_bgr_255_np_uint8[:,:,[2,1,0]]
        #     # points_tensor = results['points'].tensor
        ...
        #         )
        #     image_aug_bgr = image_aug_rgb[:,:,[2,1,0]]
        #     results['img_inputs'] = image_aug_bgr
        ...
        if 'snow_sim_image' in self.corruption_severity_dict:
            ...
            for cam_name in cam_names:
                ...
                # filename1 = results['curr']['cams']['CAM_FRONT']['data_path']
                # cv2.imwrite("/opt/data/private/codeN1/BEVDet1/Save/curruptions/snow_image/" + filename1[filename1.rfind("/") + 1:-4] + cam_name + ".jpg", img1)
                ...
                image_aug_bgr = image_aug_rgb[:,:,[2,1,0]].astype(np.float32)

                # filename1 = results['curr']['cams']['CAM_FRONT']['data_path']
                # cv2.imwrite("/opt/data/private/codeN1/BEVDet1/Save/curruptions/snow_image/" + filename1[filename1.rfind("/") + 1:-4] + cam_name + ".jpg", image_aug_bgr)
                imgs.append(mmlabNormalize(image_aug_bgr))
            imgs = torch.stack(imgs)
            results['img_inputs'] = tuple([imgs, results['img_inputs'][1], results['img_inputs'][2], results['img_inputs'][3], results['img_inputs'][4], results['img_inputs'][5]])
            # elif type(img_bgr_255_np_uint8) == list and len(img_bgr_255_np_uint8) == 5:
            #     img = []
        ...
        if 'fog_sim_image' in self.corruption_severity_dict:
            ...
            for cam_name in cam_names:
                ...
                filename1 = results['curr']['cams']['CAM_FRONT']['data_path']
                cv2.imwrite("/opt/data/private/codeN1/BEVDet1/Save/curruptions/fog_image/" + filename1[filename1.rfind("/") + 1:-4] + cam_name + ".jpg", image_aug_bgr)
        ...
        if 'rain_sim_image' in self.corruption_severity_dict:
            ...
            for cam_name in cam_names:
                ...
                image_aug_bgr = image_aug_rgb[:,:,[2,1,0]].astype(np.float32)
                # cv2.imwrite("/gongshenjian/jimingqian/BEVDet/Save/4.jpg", image_aug_bgr)
                imgs.append(mmlabNormalize(image_aug_bgr))
            imgs = torch.stack(imgs)
            results['img_inputs'] = tuple([imgs, results['img_inputs'][1], results['img_inputs'][2], results['img_inputs'][3], results['img_inputs'][4], results['img_inputs'][5]])
            # img_bgr_255_np_uint8 = results['img']
    ...
    #############################################
        ...
        if 'scene_glare_noise' in self.corruption_severity_dict:
            ...
            pl = torch.from_numpy(points_aug).float()

            import open3d as o3d
            filename = results['curr']['cams']['CAM_FRONT']['data_path']
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(pl.numpy()[:, :3])
            o3d.io.write_point_cloud("/opt/data/private/codeN1/BEVDet1/Save/curruptions/sun_lidar/" + filename[filename.rfind("/") + 1:-4] + ".ply", pcd)
        ...
        if 'fog_sim_lidar' in self.corruption_severity_dict:
            ...
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(pl.numpy()[:, :3])
            o3d.io.write_point_cloud("/opt/data/private/codeN1/BEVDet1/Save/curruptions/fog_lidar/" + filename[filename.rfind("/") + 1:-4] + ".ply", pcd)
        ...
        if 'snow_sim_lidar' in self.corruption_severity_dict:
            ...
            # pcd = o3d.geometry.PointCloud()
            # pcd.points = o3d.utility.Vector3dVector(pl.numpy()[:, :3])
            # o3d.io.write_point_cloud("/opt/data/private/codeN1/BEVDet1/Save/curruptions/snow_lidar/" + filename[filename.rfind("/") + 1:-4] + ".ply", pcd)
        ...
        if 'temporal_alignment_noise' in self.corruption_severity_dict:
            ...
            for i in range(len(cam_names)):
                # if frame 
                cam_data = results['curr']['cams'][cam_names[i]]
                filename = cam_data['data_path']
                # filename = '/opt/data/common/nuScenes_org/data_v1.0-trainval/' +  filename[53:]
                img = Image.open(filename)
                post_rot = torch.eye(2)
                ...
                # cv2.imwrite("/gongshenjian/jimingqian/BEVDet/Save/3.jpg", img)
                new_np = img[:,:,[2,1,0]].astype(np.uint8)
                image_aug_bgr = new_np[:,:,[2,1,0]].astype(np.float32)
                # cv2.imwrite("/gongshenjian/jimingqian/BEVDet/Save/4.jpg", image_aug_bgr)
                imgs.append(mmlabNormalize(image_aug_bgr))
            imgs = torch.stack(imgs)
            results['img_inputs'] = tuple([imgs, results['img_inputs'][1], results['img_inputs'][2], results['img_inputs'][3], results['img_inputs'][4], results['img_inputs'][5]])
        ...
        if 'fulltrajectory_noise' in self.corruption_severity_dict:
            ...
            if len(results['sweeps']) != 0:
                ...
                curr_sweeps = results['curr']
                fir_ego_pose = format_list_float_06(curr_sweeps['ego2global_translation'] + curr_sweeps['ego2global_rotation'])
                fir_sen_glo = format_list_float_06(curr_sweeps['lidar2ego_translation'] + curr_sweeps['lidar2ego_rotation'])
        ...
        if 'fov' in self.corruption_severity_dict:
            ...
            points_aug = fov_filter(pl.numpy(), severity)
            pl = torch.from_numpy(points_aug)
            results['points'].tensor = pl
