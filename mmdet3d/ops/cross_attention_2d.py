...
class DeformableAttention2D(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.to_de = nn.Conv2d(dim, inner_dim, 1, groups = offset_groups if group_queries else 1, bias = False)
    ...
    def forward(self, x_q, x_kv, De, return_vgrid = False):
        """
        b - batch
        h - heads
        x - height
        ...
        # queries
        if De != None:
            q = self.to_q(x_q) * self.to_de(De)
        else:
            q = self.to_q(x_q)

        # print(x_q.shape) # [1, 256, 36, 36]
        ...
        # 可视化 self.to_q 的参数
        # x_q1 = torch.randn(1, 256, 36, 36).cuda()
        # self.visualize_weights(self.to_q(x_q), self.to_de(De))
    ...
    def visualize_weights(self, x_q1, De):
        # weights = self.to_q.weight.data.cpu().numpy()  # 获取 self.to_q 的权重 [512, 32, 1, 1]
        # weights_min = weights.min()
        # weights_max = weights.max()
        # weights = (weights - weights_min) / (weights_max - weights_min) * 255
        # print(weights)

        import numpy as np
        import cv2
        # print(De.shape)
        # weights = np.where(weights > 0, 1, 0) *255 
        # weights = np.mean(weights.squeeze(2).squeeze(2), axis=0)
        weights = (De * x_q1).cpu().numpy()
        weights = np.mean(weights.squeeze(0), axis=0) # [36, 36]
        # weights_min = weights.min()
        # weights_max = weights.max()
        # weights = (weights - weights_min) / (weights_max - weights_min) * 255
        bev_features = weights
        center_x, center_y = bev_features.shape[0] // 2, bev_features.shape[1] // 2
        x = np.arange(bev_features.shape[0])
        y = np.arange(bev_features.shape[1])
        X, Y = np.meshgrid(x, y)
        ...
        depth_features = np.zeros(max_distance + 1)

        for d in range(max_distance + 1):
            depth_features[d] = bev_features[distance_matrix == d].mean()

        # 生成深度索引
        depth_indices = np.arange(0, max_distance + 1)

        # 生成深度索引
        depth_indices = np.arange(1, len(depth_features) + 1)

        # 绘制折线图
        plt.figure(figsize=(10, 6))
        plt.plot(depth_indices, depth_features, marker='o', linestyle='-', color='b')
        ...
        times += 1
        # plt.show()
        np.save("/opt/data/private/codeN1/BEVDet1/Save/weight/weight_npy/" + str(times) + '.npy', depth_features)
        # plt.savefig('/opt/data/private/codeN1/BEVDet1/Save/weight/zhexian' + str(times) + '.png')
        
        # cv2.imwrite('/opt/data/private/codeN1/BEVDet1/Save/weight/visualization_of_to_q_weights.png', weights)
        # plt.imshow(weights, cmap='hot')
        # plt.savefig('/opt/data/private/codeN1/BEVDet1/Save/weight/visualization_of_to_q_weights.png')
        # plt.axis('off')
