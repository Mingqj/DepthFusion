...
class DepthFusionTinyHead(TransFusionHead):
    def __init__(self,
                 img_feat_dim=128,
                 feat_bev_img_dim=32,
                 ...
                 dense_fuse_layers=2,
                 **kwargs):
        super(DepthFusionTinyHead, self).__init__(**kwargs)
        ...
        self.down_lc1 = nn.Sequential(
            nn.Conv2d(self.dim_channel, self.dim_channel, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(self.dim_channel, eps=1e-5, momentum=0.1),
            nn.ReLU(True),
            nn.Conv2d(self.dim_channel, 128, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(128, eps=1e-5, momentum=0.1),
            nn.ReLU(True),
        )

        self.heatmap_attention_head = nn.Sequential(
        ...
        position_threshold = int(dis_width * 0.9)
    ...
    def forward_single(self, inputs, img_inputs, bev_feat_img, img_metas):
        ...
        ###################################################################
        dense_fuse_feat_attention_flatten = dense_fuse_feat_attention.view(batch_size, dense_fuse_feat_attention.shape[1], -1)
